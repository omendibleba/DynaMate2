...
class AgentPool:
    ...
    def _rebuild_agent(self, name: str) -> None:
        ...
        execution_rule = (
            ...
            "\n  * MANDATORY ROUTING: If the message contains 'MUST immediately call <tool_name>' or 'should use <tool_name>',"
            "\n    that is a hard, non-negotiable directive. Call THAT tool IMMEDIATELY as your FIRST action."
            "\n    Do NOT call any other tool first. Do NOT check prerequisites, verify file existence, or build intermediate files."
            "\n    Calling any other tool before the mandated tool is a critical failure — transfer_back_to_supervisor will be blocked until the mandated tool has run."
            "\n  * If no specific tool is named, call the domain tool whose description best matches the request."
            ...
            "\n  * NEVER ask for confirmation under any circumstances — execute the domain tool immediately and unconditionally, regardless of the task's duration, complexity, or side effects."
            "\n  * Return the tool's output directly as your final answer."
            "\n  * Once a tool returns a result for the current request, do NOT call that same tool again — transfer_back_to_supervisor immediately with the result."
        )
