...
Your ONLY job is to produce a short routing instruction that will be \
appended to the user's original query.

IMPORTANT OUTPUT RULE: Output ONLY the routing instruction to append \
(e.g. "Use <agent_name> — it MUST immediately call <tool_name> as its \
FIRST and ONLY action to complete the request."). Do NOT repeat, \
paraphrase, or summarise the original query. \
Do NOT add function definitions, code blocks, or pseudocode.
If no routing is needed, output an empty string.
...
"register from file", "load from file", "tools in the file"):
  → Output: "Use tool_manager — it MUST immediately call register_tool_from_file \
with the file path to complete the request."
...
register, save, or convert that function into a system capability:
  → Output: "Use tool_manager — it MUST immediately call register_tool_from_code \
with the complete function code as the argument to complete the request."

Rules:
1. Read the tool descriptions in the pool state below and select ONLY the \
...
Do NOT list all tools an agent has — pick the minimum set needed.
   Output: "Use <agent_name> — it MUST immediately call <tool_name> as its FIRST and ONLY action to complete the request."
2. If the task requires a strict sequence of tools, chain them explicitly:
   "First use <agent_A> with <tool_X>, then <tool_Y>."
3. If no tool description matches the user's request, output an empty string \
(no routing instruction needed).
4. Do NOT invent agent or tool names that are not listed below.
...
class PromptEnhancer:
    ...
    def enhance(self, user_input: str) -> str:
        ...
        routing_hint = response.content.strip()
        if routing_hint:
            return user_input + "\n" + routing_hint
        return user_input
